...
def id_terminator(moon_img, circle_r, circle_ctr, phase_scale_factor=4, theta_steps = 360, a_max = 0):
    ...
    for step_num in range(theta_steps):
        theta = step_num * (np.pi)/theta_steps
        grad = gen_grad_field(img_shape, ctr, b, theta)
        for a in range(1, a_max+1):
            ellipse = cv.ellipse(np.zeros(img_shape), (round(ctr[0]), round(ctr[1])), (round(a), round(b)), np.degrees(-theta), 0, 360, color=1, thickness=1, lineType=cv.LINE_AA)
    ...
    # refinements
    zone = 8
    peak = hough_space[ellipse_id[0]-zone:ellipse_id[0]+zone+1, ellipse_id[1]-zone:ellipse_id[1]+zone+1]
    ...
    ellipse_theta = (ellipse_id[0] + cent_row) * (np.pi)/theta_steps

    # fig1, axes = plt.subplots(2,3)
    # utility.plotImg(axes, (0,2), peak)
    # utility.plotImg(axes, (0,0), moon_img)
...
original_img = cv.imread("images/test-2.jpg")
...
t2 = time.time()
ellipse_a, ellipse_theta = id_terminator(moon_img, circle_r, circle_ctr, phase_scale_factor=4)
t3 = time.time()
...
# utility.plotImg(axes, 0, edges)
# utility.plotImg(axes, 1, hough_space[:,:,circle_id[2]])
# plotImg(axes, 1, kernel)

colorized = moon_img
cv.circle(colorized, circle_ctr, 2, (0, 255, 0), -1)
cv.circle(colorized, circle_ctr, circle_r, (0, 255, 0), 1)
cv.ellipse(colorized, circle_ctr, (round(ellipse_a), circle_r), np.degrees(-ellipse_theta), 0, 360, (0, 0, 255), 1)
